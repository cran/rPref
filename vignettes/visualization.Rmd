...
The tuples having the same level are placed on the same row. Here we have 4 rows, corresponding to the 4 levels which were selected using `top_level = 4`. 

To get a graph with more meaningful labels we create a label string containing all relevant columns from the data set `mtcars` for the preference `p` and the row number `num`:
